      ...
      {props.isLoading
        ? (
          <Col className="items-center self-auto justify-center w-full h-full">
            <RenderMarkdown className="prose text-md font-extrabold text-base-content opacity-30 text-center">
              {props?.loadingState?.title}
            </RenderMarkdown>
            <RenderMarkdown className="prose text-xs font-extrabold text-base-content opacity-30 text-center">
              {props?.loadingState?.description}
            </RenderMarkdown>
          </Col>
        )
        : (
          <Col className="lg:px-12 items-start justify-start overflow-auto h-full">
            {props.showDetails
              ? (
                <ConversationDetails
                  markdown={props.detailsMarkdown}
